...
class My {
	public function Settings() {
		...
		if(isset($_POST['submit'])) {
			...
			$GetDataComponent = mysqli_query($link,"SELECT passwd FROM members WHERE member_id = ".$owner."");
			$executesql = mysqli_fetch_assoc($GetDataComponent);
			
			$firstname			=	strip_tags(mysqli_real_escape_string($link,$_POST['firstname']));
			$lastname			=	strip_tags(mysqli_real_escape_string($link,$_POST['lastname']));
			
			$mail				=	strip_tags(mysqli_real_escape_string($link,$_POST['mail']));
			$oldpass			=	strip_tags(mysqli_real_escape_string($link,$_POST['oldpass']));
			$newpass			=	strip_tags(mysqli_real_escape_string($link,$_POST['newpass']));
			
			$measurement		=	strip_tags(mysqli_real_escape_string($link,$_POST['measurement']));
			$currency			=	strip_tags(mysqli_real_escape_string($link,$_POST['currency']));			
